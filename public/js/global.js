    ...
    socket.on('loggedInUser', function(users){
        ...
        for(var i = 0; i < users.length; ++i){
            if(friend.indexOf(users[i].name) > -1){
                ...
                + '<a id="val" href="/chat/' + userName.replace(/ /g, "-") + '.' + name.replace(/ /g, "-") + '"><h3 style="padding-top: 15px; color: grey; font-size: 14px;">' + '@'+ users[i].name + '<span class="fa fa-circle online_friend"></span></h3></a></p>';
                ol.append(list);
            }
        }
